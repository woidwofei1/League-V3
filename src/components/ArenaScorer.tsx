  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        <div className={`absolute inset-0 transition-opacity duration-300 ${lastScorer === 'bachi' ? 'opacity-100' : 'opacity-0'
          }`}>
          <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-cyan-500/30 to-transparent" />
        </div>
        <div className={`absolute inset-0 transition-opacity duration-300 ${lastScorer === 'crimebaker' ? 'opacity-100' : 'opacity-0'
          }`}>
      ...
      <div className="absolute top-0 left-0 right-0 z-50 p-4">
        <div className="flex items-center justify-between">
          <button
            onClick={handleClose}
            className="p-3 rounded-full bg-white/5 text-white/50 hover:text-white hover:bg-white/10 transition-all min-w-[44px] min-h-[44px] flex items-center justify-center"
          ...
          <div className="text-center">
            ...
            <div className="flex items-center gap-1 bg-white/5 rounded-full p-1">
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-mono uppercase transition-all ${mode === 'live' ? 'bg-white/20 text-white' : 'text-white/40 hover:text-white/60'
                  }`}
              >
                <Play size={12} /> Live
              </button>
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-mono uppercase transition-all ${mode === 'quick' ? 'bg-white/20 text-white' : 'text-white/40 hover:text-white/60'
                  }`}
              >
                <Edit3 size={12} /> Quick
              </button>
          ...
          {mode === 'live' ? (
            <button
              ...
              className="p-3 rounded-full bg-white/5 text-white/50 hover:text-white hover:bg-white/10 disabled:opacity-30 transition-all min-w-[44px] min-h-[44px] flex items-center justify-center"
        ...
        <div className="flex justify-center gap-3 mt-4">
          <div className="flex items-center gap-1">
            {[...Array(SETS_TO_WIN)].map((_, i) => (
              <div
                key={`bachi-${i}`}
                className={`w-3 h-3 rounded-full transition-all ${i < setsWon.bachi ? 'bg-cyan-500' : 'bg-white/10'
                  }`}
              />
            ))}
          </div>
          ...
          <div className="flex items-center gap-1">
            {[...Array(SETS_TO_WIN)].map((_, i) => (
              <div
                key={`crime-${i}`}
                className={`w-3 h-3 rounded-full transition-all ${i < setsWon.crimebaker ? 'bg-pink-500' : 'bg-white/10'
                  }`}
              />
            ))}
          </div>
      ...
      {mode === 'live' && (
        <>
          {/* BACHI Zone (Top) */}
          <motion.button
            className="absolute top-0 left-0 right-0 h-[45%] flex flex-col items-center justify-center pt-24 overflow-hidden"
            onPointerDown={(e) => {
              e.preventDefault();
              scorePoint('bachi');
            }}
          ...
          >
            ...
            {momentum.player === 'bachi' && momentum.count >= 3 && (
              <motion.div
                className="absolute inset-0 bg-gradient-to-b from-cyan-500/30 via-cyan-500/10 to-transparent"
                initial={{ opacity: 0 }}
          ...
          <div className="absolute top-1/2 left-0 right-0 -translate-y-1/2 z-40 pointer-events-none">
            <div className="relative">
              ...
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                <motion.div
                  className={`w-16 h-16 rounded-full bg-black border-2 flex items-center justify-center ${advantage === 'bachi' ? 'border-cyan-500/50' :
              ...
              {isDeuce && (
                <motion.p
                  ...
                  className={`absolute left-1/2 -translate-x-1/2 top-12 font-mono text-xs uppercase tracking-widest ${advantage === 'bachi' ? 'text-cyan-400' :
                    advantage === 'crimebaker' ? 'text-pink-400' :
                      'text-yellow-400'
                    }`}
                >
          ...
          <motion.button
            className="absolute bottom-0 left-0 right-0 h-[45%] flex flex-col items-center justify-center pb-24 overflow-hidden"
            onPointerDown={(e) => {
              e.preventDefault();
          ...
          >
            {/* Momentum Glow Effect */}
            {momentum.player === 'crimebaker' && momentum.count >= 3 && (
              <motion.div
                className="absolute inset-0 bg-gradient-to-t from-pink-500/30 via-pink-500/10 to-transparent"
                initial={{ opacity: 0 }}
                animate={{ opacity: [0.3, 0.6, 0.3] }}
                transition={{ duration: 1, repeat: Infinity }}
              />
        ...
        <div className="absolute inset-0 pt-32 pb-8 px-4 overflow-auto z-30">
          <div className="max-w-md mx-auto space-y-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 flex items-center justify-center gap-2">
                    ...
                    <span className={`text-3xl font-display tabular-nums min-w-[3ch] text-center ${bachiWon ? 'text-cyan-400' : 'text-white/60'
                      }`}>
                      {set.bachi}
                    </span>
                    <button
                  ...
                  <div className="flex-1 flex items-center justify-center gap-2">
                    ...
                    <span className={`text-3xl font-display tabular-nums min-w-[3ch] text-center ${crimebakerWon ? 'text-pink-400' : 'text-white/60'
                      }`}>
                      {set.crimebaker}
                    </span>
                    <button
            ...
            <motion.button
              ...
              className={`w-full py-4 rounded-2xl font-display uppercase tracking-wider text-lg flex items-center justify-center gap-2 transition-all ${canSubmitQuick
      ...
      <AnimatePresence>
        {showSetWin && !matchWinner && (
          ...
          >
            <div className={`absolute inset-0 ${showSetWin === 'bachi' ? 'bg-cyan-900/80' : 'bg-pink-900/80'
              } backdrop-blur-sm`} />
            ...
            >
              <motion.div
                ...
                animate={{ y: 0 }}
              >
                <p className={`text-7xl font-display uppercase tracking-tight ${showSetWin === 'bachi' ? 'text-cyan-400' : 'text-pink-400'
        ...
        {matchWinner && (
          ...
          >
            ...
            <motion.div
              ...
              className={`absolute w-full h-full ${matchWinner === 'bachi'
            ...
            <div className="relative z-10 text-center px-8">
              <motion.div
                ...
                className={`w-28 h-28 rounded-full flex items-center justify-center mx-auto mb-8 ${matchWinner === 'bachi' ? 'bg-cyan-500' : 'bg-pink-500'
                  }`}
              ...
              <motion.p
                ...
                className={`text-3xl font-display uppercase tracking-widest ${matchWinner === 'bachi' ? 'text-cyan-400' : 'text-pink-400'
                  }`}
              >
                Wins!
              </motion.p>
